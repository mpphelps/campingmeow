...
3. **Watches.** A user creates a watch on up to **20 facilities** (across any parks) with a shared date pattern: check-in weekdays, nights (1–7), and optional check-in date bounds. Example: "any Friday, 2 nights, June–August — at Moro Campground or San Mateo." Unbounded watches cover the full booking window and roll forward with it; a bounded watch whose end date has passed is auto-deactivated by the scanner.
...
9. **Admin panel.** Admin-only page: list users, see each user's watches, remove/ban users, view the health metrics above, run a catalog sync, and start an availability sweep of either the watched campgrounds or the entire catalog (~500 campgrounds, ~3 hours). Sweeps run in the background with live progress.
